...
(function() {
  var AudioContext, analyser, c_range, context, e, h, hue, hues, input, interval, ip, user, v_range, _i, _len;
  ...
  $(function() {
    ...
    getFreq = function() {
      var buffer, highSum, i, lowSum, s, status, volume, volumeSum, _j;
      ...
      for (h in hues) {
        s = hues[h];
        if (volume > v_range && !s["lightSwitch"]) {
          hue.lightTrriger(h, true).then(function(result) {
            console.log('light on');
            return s["lightSwitch"] = true;
          }).fail(function(err) {
            return console.log(err);
          });
        } else if (volume < v_range && s["lightSwitch"]) {
          hue.lightTrriger(h, false).then(function(result) {
            console.log('light off');
            return s["lightSwitch"] = false;
          }).fail(function(err) {
            return console.log(err);
          });
        }
      ...
      for (h in hues) {
        s = hues[h];
        if (status > c_range && s["lightColor"] === "blue") {
          hue.changeColor(h, 0).then(function(result) {
            console.log('change to red');
            return s["lightColor"] = "red";
          }).fail(function(err) {
            return console.log(err);
          });
        } else if (status < c_range && s["lightColor"] === "red") {
          hue.changeColor(h, 46920).then(function(result) {
            console.log('change to blue');
            return s["lightColor"] = "blue";
          }).fail(function(err) {
            return console.log(err);
          });
        }
      ...
      $status.val(status > c_range ? "red" : "blue");
